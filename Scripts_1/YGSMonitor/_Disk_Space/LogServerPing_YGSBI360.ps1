...
# foreach ($Server in $ServerName) {
    ...
    if (test-Connection -ComputerName $target -Count 2 -Quiet ) { 
		...
		$SQLQuery = "INSERT INTO MonitoringLog (Name, MeasureName, MeasureValue) VALUES ('YGSBI360','ping','true')"
		...
		{ 
		$SQLQuery = "INSERT INTO MonitoringLog (Name, MeasureName, MeasureValue) VALUES ('YGSBI360','ping','false')"
		$SqlCmd.CommandText = $SqlQuery
		$SqlCmd.Connection = $SqlConnection
