...
/*
Get the edition and version of SQL Server
source: https://gallery.technet.microsoft.com/Determining-which-version-af0f16f6
date: 5/31/2017
...
SELECT  @SPInfo = 
		...
		CASE @ProductName
			    WHEN 'SQL Server 2016' THEN 
											CASE @ProductVersion
												WHEN '13.0.4422.0' THEN 'You have already installed the latest cumulative update.' 
												ELSE 'Install the latest Cumulative Update (CU) of SP1:  CU2, <https://support.microsoft.com/en-us/kb/4013106>'
											END
				WHEN 'SQL Server 2014' THEN 
											CASE @ProductVersion
				...
				WHEN 'SQL Server 2012' THEN
											CASE @ProductVersion
												WHEN '11.0.6594.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP3:  CU8, <https://support.microsoft.com/en-us/kb/4013104>'
											END
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP3') AND (@ProductVersion < '11.0.6594.0')
	BEGIN
		SET @CumulativeUpdate = 'CU8'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4013104';
	END
	SELECT  @OtherProduct = 
			CASE 
				WHEN @ProductLevel = 'RTM' THEN ' If you want to upgrade to higher edition, please intall service pack.'
...
BEGIN
	...
	--2016
	IF (@ProductLevel = 'RTM') AND (@ProductVersion < '13.0.2197.0')
	BEGIN
		SET @CumulativeUpdate = 'CU5'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4013105';
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '13.0.4422.0')
	BEGIN
		SET @CumulativeUpdate = 'CU2'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/4013106';
	END			
END
